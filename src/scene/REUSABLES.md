...
| Shared ripple clock (`focusRef.rippleAt`)                                          | One `performance.now()` instant, two layers: canvas shader uniforms and DOM labels both derive "what is lit now" from it — layers can never disagree about a travelling change.                                                                                        |
